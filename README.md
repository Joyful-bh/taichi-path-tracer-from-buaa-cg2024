...
它既可以作为可直接运行的离线渲染器，也可以用于研究 Taichi 后端上的光线追踪算法、GPU 性能瓶颈和不同积分器架构。场景由 YAML 描述，支持 CPU、CUDA、Vulkan、与 Metal后端。
...
> 如果你的设备是装有 M 芯片的 Macbook, 请使用 `Mac_M_chips`分支, 并参考其中的 README 完成环境配置和代码运行.
